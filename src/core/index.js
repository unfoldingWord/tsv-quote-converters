...
import { readTsv, tsvRecordToString } from '../utils/tsv.js';
import { rejigAlignment } from '../utils/rejig_alignment.js';
import { slimSourceTokens } from '../utils/tokens.js';
import { UWProskomma } from './uw-proskomma.js';
...
const getDocuments = async (pk, book, dcsUrl = "https://git.door43.org") => {
  ...
  ol_bible = BibleBookData?.[book]?.testament === "old" ? "hbo_uhb" : "el-x-koine_ugnt";
  if (!ol_bible) {
  ...
  for (const [org, lang, abbr, baseURL] of baseURLs) {
    ...
    if (abbr === 'ult') {
      // Preprocess x-occurrence,x-occurrences,x-content into x-align="content:occurrence:occurrences" for easier handling later
...
export default function ConvertEnglishQuotesToOLQuotes(book, tsvContent) {
  let output = '';
  let errors = '';

  testament = BibleBookData[book.toLowerCase()]?.testament;
  if (! testament) {
    console.error(`ERROR: Book ${book} not a valid Bible book`);
    return;
  }
  ...
  .then(async () => {
    ...
    const tsvRecords = readTsv(tsvContent);
    for (const tsvRecord of tsvRecords) {
      ...
      if (! tsvRecord.ref || ! tsvRecord.quote || ! tsvRecord.occurrence || tsvRecord.ref == "Reference") {
        if (tsvRecord.ref) {
            output += tsvRecordToString(tsvRecord);
        }
        continue;
      }
      ...
      if ('data' in resultObject) {
        ...
        console.log(`${book}_${cv} ►${tsvRecord.quote}◄ “${getTidiedData(resultObject.data)}”`);
        tsvRecord.quote = getTidiedData(resultObject.data);
        tsvRecordToString(tsvRecord);
      } else {
